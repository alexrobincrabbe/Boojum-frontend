...
import {
  ...
  triggerWordCounterAnimation,
} from "../utils/borderAnimation";
...
import { wordCountDefault, wordsByLengthDefault } from "../defaults/defaults";
...
export function useWordTracking(
  ...
  const [wordCounts, setWordCounts] = useState<Record<string, number>>(wordCountDefault);
  const [wordCountMax, setWordCountMax] = useState<Record<string, number>>(wordCountDefault);
  const [wordsByLength, setWordsByLength] = 
  useState<Record<string, string[]> | Record<string, Record<string, any>>>(wordsByLengthDefault);
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (
        ...
        if (
          firstWordData &&
          typeof firstWordData === "object" &&
          "score" in firstWordData
        ) {
  ...
  const bestWordRef = useRef<{ word: string; score: number }>({
    word: "",
    score: 0,
  });
  ...
  const initializeWordLists = useCallback(
    (
      newWordsByLength: Record<string, string[]>,
      passedGameState?: GameState | null,
      sendJson?: (message: any) => void
    ) => {
      ...
      const isNewGame =
        currentGameRoundId &&
        currentGameRoundId !== previousGameRoundIdRef.current;

      // Get username from the state being used
      let username: string | null = null;
      if (stateToUse?.currentPlayerId && stateToUse?.players) {
        const currentPlayer = stateToUse.players.find(
          (p) => p.id === stateToUse.currentPlayerId
        );
        ...
        if (restored) {
          ...
          if (
            ...
            const whichWordsFound = stateToUse.boardWords.map((word) =>
              restored.wordsFoundArray.some((foundWord) => foundWord === word)
                ? 1
                : 0
            );
          ...
          // No saved state for this new game - ALWAYS reset for a new game
          // (The "preserve existing scores" logic was for reconnection to the SAME game,
          // but for a NEW game, we should always start fresh)
          // Reset to defaults for new game
          setWordCounts(wordCountDefault);
  ...
  const submitOneShotWord = useCallback(
    (word: string, time: number, sendJson: (message: any) => void) => {
      ...
      const whichWordsFound =
        gameState?.boardWords?.map((w) => (w === word ? 1 : 0)) || [];
